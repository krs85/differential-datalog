    ...
    P::Convert: Send + DDlogConvert,
{
    ...
    /// Same functionality as add_sink_accumulator() except that it also takes mock
    /// data to add to the accumulator.
    /// Used for testing.
    pub fn add_sink_accumulator_with_testing_data(
        &mut self,
        rel_ids: BTreeSet<RelId>,
        server: &mut DDlogServer<P>,
    ) -> Result<(), String> {
        if !self._sinks.contains_key(&rel_ids) {
            ...
            set.insert(1);
            map.insert(1, set);
            accumulator.add_mock_state(map);
    ...
    pub fn get_file_sink_record(&self, sink: &Sink, rel_ids: BTreeSet<RelId>) -> Result<String, String> {
        let (_, _, sink_map) = self._sinks.get(&rel_ids).unwrap();
        let (sink_realization, _) = sink_map.get(sink).unwrap();
        let mut content = String::new();
        match *sink_realization {
            SinkRealization::File(file_sink) => {
                let mut guard = file_sink.lock().unwrap();
                Ok(guard.get_file_contents())
            },
            _ => Err(String::from("Error: Function is for file sinks, but a tcp sink was used"))
        }
    }
